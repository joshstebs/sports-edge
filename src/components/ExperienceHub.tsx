...
export type ExperienceId =
  | 'parlay' | 'performance' | 'provenance' | 'prop-lab' | 'voice'
  | 'ledger' | 'pregame' | 'slip' | 'research' | 'landing';
...
const META: Array<{ id: ExperienceId; code: string; label: string; hint: string }> = [
  ...
  { id: 'prop-lab', code: 'SE-04', label: 'Prop Lab', hint: 'Official market-specific game logs' },
  { id: 'voice', code: 'SE-05', label: 'Voice Mode', hint: 'Hands-free analyst' },
  { id: 'ledger', code: 'SE-06', label: 'Receipts', hint: 'Tracked outcomes' },
  { id: 'pregame', code: 'SE-07', label: 'Pregame', hint: 'Live research brief' },
...
function LiveBadges({ health, healthInfo }: { health: HealthState; healthInfo: HealthInfo | null }) {
  ...
  const configured = Object.values(healthInfo?.sources ?? {}).filter((value) =>
    typeof value !== 'object' || value === null || !('available' in value) || (value as { available?: boolean }).available !== false
  ).length;
  return (
    <div className="grid grid-cols-2 gap-2">
      ...
      <div className="rounded-xl border border-sky2/25 bg-sky2/[0.04] px-3 py-2.5">
        <div className="text-[10px] font-black uppercase tracking-[0.12em] text-sky2">Verified-or-withheld</div>
...
function EmptyVerified({ onSend }: { onSend: (text: string) => void }) {
  return (
    <div className="rounded-2xl border border-line bg-panel/75 p-6 text-center">
      ...
      <p className="mx-auto mt-2 max-w-md text-xs leading-relaxed text-frost2">Generate a recommendation first. Empty states stay empty instead of using fake betting data.</p>
...
function LegCard({ leg, index = 0, condensed = false }: { leg: SgpLeg; index?: number; condensed?: boolean }) {
  ...
  const priced = typeof leg.odds === 'number';
  return (
      ...
      <div className="flex gap-3 p-3.5 sm:p-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-3 grid grid-cols-4 gap-2 border-t border-line/70 pt-2.5">
            ...
            <Metric label="Odds" value={displayOdds(leg)} tone={priced ? 'green' : undefined} />
            <Metric label="Model" value={confidence == null ? '—' : `${Math.round(confidence)}%`} tone="cyan" />
            <Metric label="EV" value={displayEv(leg)} tone={priced ? 'green' : undefined} />
          </div>
        </div>
      </div>
      <div className="flex flex-wrap gap-1.5 border-t border-line/60 bg-black/15 px-3.5 py-2.5">
        <span className="rounded-md border border-sky2/20 bg-sky2/[0.035] px-2 py-1 text-[8px] font-bold uppercase tracking-[0.08em] text-sky2">{priced ? 'Priced leg' : 'Price unavailable'}</span>
        <span className="rounded-md border border-sky2/20 bg-sky2/[0.035] px-2 py-1 text-[8px] font-bold uppercase tracking-[0.08em] text-sky2">Availability screened</span>
...
function ParlayView({ legs, health, healthInfo, onSend }: { legs: SgpLeg[]; health: HealthState; healthInfo: HealthInfo | null; onSend: (text: string) => void }) {
  ...
  return <Shell><Brand/><div className="mt-4"><LiveBadges health={health} healthInfo={healthInfo}/></div><div className="mt-4 rounded-2xl border border-line bg-panel/70 p-4"><p className="text-[9px] font-bold uppercase tracking-[0.12em] text-frost2">You</p><p className="mt-1.5 text-lg font-semibold leading-snug text-head">Build me a 3-leg parlay using only verified lines.</p></div><div className="mt-3 rounded-2xl border border-edge/15 bg-panel/60 p-3.5"><p className="mb-3 text-[9px] font-black uppercase tracking-[0.12em] text-edge">SportsEdge AI · structured response</p>{visible.length ? <div className="space-y-2.5">{visible.map((leg,index)=><LegCard key={`${leg.selection}-${index}`} leg={leg} index={index} condensed/>)}</div> : <EmptyVerified onSend={onSend}/>}</div><div className="sticky bottom-0 mt-3 flex items-center justify-between rounded-2xl border border-edge/25 bg-panel/95 p-4 backdrop-blur"><div><p className="text-[9px] uppercase tracking-widest text-frost2">Parlay slip</p><p className="mt-1 font-mono text-xl font-black text-edge">{combinedOdds(visible)}</p></div><button onClick={()=>onSend('Review my current parlay slip for risk, correlation, and line verification.')} className="rounded-xl bg-edge px-4 py-2.5 text-[10px] font-black uppercase tracking-wider text-ink">Review slip</button></div></Shell>;
}

function PerformanceView() {
  const [data,setData]=useState<PerformanceSummary|null>(null); const [error,setError]=useState<string|null>(null);
  useEffect(()=>{let cancelled=false;fetchPerformance().then(v=>{if(!cancelled)setData(v.performance)}).catch(e=>{if(!cancelled)setError(e instanceof Error?e.message:'Performance unavailable')});return()=>{cancelled=true}},[]);
  const pct=(v:number|null|undefined)=>v==null?'—':`${v.toFixed(1)}%`;
  return <Shell><Brand/><div className="mt-5 flex items-end justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.15em] text-frost2">Model Performance</p><h2 className="mt-1 text-2xl font-black text-head">Transparent. Auditable.</h2></div><span className="rounded-lg border border-edge/25 bg-edge/5 px-2 py-1 text-[8px] font-black uppercase text-edge">{data?.storage.durable?'Durable storage':'Performance ledger'}</span></div>{error?<div className="mt-4 rounded-xl border border-danger/30 bg-danger/5 p-4 text-xs text-danger">{error}</div>:null}<div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><Stat label="Graded picks" value={data?String(data.overall.graded):'…'}/><Stat label="Win rate" value={data?pct(data.overall.winRate):'…'} tone="cyan"/><Stat label="ROI" value={data?pct(data.overall.roiPct):'…'} tone="green"/><Stat label="Priced legs" value={data?String(data.overall.priced):'…'} tone="cyan"/></div><div className="mt-3 rounded-2xl border border-line bg-panel/70 p-4"><div className="flex items-center justify-between"><p className="text-[10px] font-black uppercase tracking-wider text-frost">Performance by sport</p><span className="font-mono text-xs text-edge">7D ROI {data?pct(data.last7.roiPct):'…'}</span></div><div className="mt-4 space-y-3">{(data?.bySport.slice(0,4)??[]).map(row=><div key={row.sport} className="grid grid-cols-[64px_1fr_46px] items-center gap-2"><span className="text-[9px] font-bold text-frost2">{row.sport}</span><div className="h-2 overflow-hidden rounded-full bg-panel2"><div className="h-full rounded-full bg-gradient-to-r from-edge to-sky2" style={{width:`${Math.max(2,Math.min(100,row.winRate??0))}%`}}/></div><span className="text-right font-mono text-[9px] text-head">{pct(row.winRate)}</span></div>)}</div></div><div className="mt-3 overflow-hidden rounded-2xl border border-line bg-panel/70"><div className="border-b border-line px-4 py-3 text-[10px] font-black uppercase tracking-wider text-frost">Recent graded predictions</div><div className="divide-y divide-line/70">{data?.recent.slice(0,6).map(row=><div key={`${row.predictionId}-${row.selection}`} className="grid grid-cols-[1fr_auto] gap-3 px-4 py-3"><div className="min-w-0"><p className="truncate text-xs font-semibold text-head">{row.selection}</p><p className="mt-0.5 text-[9px] text-frost2">{row.sport} · {row.matchup}</p></div><span className={cx('self-center rounded-md px-2 py-1 text-[9px] font-black uppercase',row.outcome==='won'?'bg-edge/10 text-edge':row.outcome==='lost'?'bg-danger/10 text-danger':'bg-warn/10 text-warn')}>{row.outcome}</span></div>)??<div className="p-4 text-xs text-frost2">Loading graded history…</div>}</div></div></Shell>;
}

function ProvenanceView({ leg, onSend }: { leg?: SgpLeg; onSend:(text:string)=>void }) {
  return <Shell centered><Brand/><h2 className="mt-8 text-center text-4xl font-black leading-[.95] tracking-[-.05em] text-head sm:text-6xl">Every number<br/><span className="text-edge">has a source.</span></h2><div className="mt-7 w-full">{leg?<LegCard leg={leg}/>:<EmptyVerified onSend={onSend}/>}</div><div className="mt-4 grid w-full grid-cols-2 gap-2 sm:grid-cols-5">{[['Odds','Price when available'],['Availability','Roster/injury screening'],['Games','Official data'],['Injuries','Current status'],['Model','Versioned evidence']].map(([a,b])=><div key={a} className="rounded-xl border border-line bg-panel/60 p-3 text-center"><p className="text-[8px] font-black uppercase text-sky2">{a}</p><p className="mt-1 text-[9px] text-frost2">{b}</p></div>)}</div><button onClick={()=>onSend('Show the full provenance and evidence for my strongest current pick.')} className="mt-5 rounded-xl border border-edge/30 bg-edge/10 px-5 py-3 text-xs font-black text-edge">Ask SportsEdge</button></Shell>;
}

function PropLabView({ leg, onSend }: { leg?: SgpLeg; onSend:(text:string)=>void }) {
  const [research,setResearch]=useState<PlayerResearchResponse|null>(null); const [error,setError]=useState<string|null>(null);
  const name=playerName(leg); const market=String(leg?.market??'').trim();
  useEffect(()=>{let cancelled=false;setResearch(null);setError(null);if(!leg?.sport||!name||!market)return;fetchPlayerResearch(leg.sport,name,market,10).then(v=>{if(!cancelled)setResearch(v)}).catch(e=>{if(!cancelled)setError(e instanceof Error?e.message:'Official game log unavailable')});return()=>{cancelled=true}},[leg?.sport,name,market]);
  if(!leg)return <Shell><Brand/><div className="mt-6"><EmptyVerified onSend={onSend}/></div></Shell>;
  const confidence=typeof leg.confidence==='number'?leg.confidence:null;
  const observations=research?.observations??[]; const numericLine=typeof leg.line==='number'?leg.line:Number(leg.line); const line=Number.isFinite(numericLine)?numericLine:null;
  const max=Math.max(1,line??0,...observations.map(o=>o.value));
  return <Shell><Brand/><div className="mt-5 flex gap-4 rounded-2xl border border-line bg-panel/70 p-4"><PlayerHeadshot sport={leg.sport} name={name} className="h-28 w-28 shrink-0"/><div className="min-w-0 flex-1"><p className="text-[9px] font-black uppercase tracking-wider text-edge">Player Prop Laboratory</p><h2 className="mt-1 text-2xl font-black tracking-tight text-head">{name||'Structured athlete'}</h2><p className="mt-1 text-xs text-frost2">{leg.game||leg.sport||'Current event'}</p><p className="mt-3 text-[9px] text-frost2">{research?.source||leg.model_source||'Loading official market history…'}</p></div></div><div className="mt-3 grid grid-cols-3 gap-2"><Stat label="Market line" value={leg.line!=null?String(leg.line):'—'}/><Stat label="Model prob" value={confidence==null?'—':`${Math.round(confidence)}%`} tone="cyan"/><Stat label="EV edge" value={displayEv(leg)} tone="green"/></div><div className="mt-3 rounded-2xl border border-line bg-panel/70 p-4"><div className="flex items-center justify-between gap-3"><p className="text-[9px] font-black uppercase tracking-wider text-frost2">Official recent {research?.market||market||'market'} results</p><span className="text-[8px] text-frost2">{research?.season||''}</span></div>{error?<p className="mt-3 text-xs text-danger">{error}</p>:observations.length?<><div className="relative mt-4 grid h-36 grid-cols-10 items-end gap-1.5 border-b border-line/70 pb-5">{line!=null?<div className="pointer-events-none absolute inset-x-0 border-t border-dashed border-sky2/60" style={{bottom:`${20+(line/max)*116}px`}}><span className="absolute right-0 -top-4 text-[8px] font-black text-sky2">LINE {line}</span></div>:null}{observations.slice(0,10).reverse().map((obs,index)=>{const over=line==null?true:obs.value>line;return <div key={`${obs.gameId??obs.date}-${index}`} className="group relative flex h-full items-end"><div className={cx('w-full min-h-1 rounded-t-md border border-white/5',over?'bg-edge/75':'bg-frost2/35')} style={{height:`${Math.max(4,(obs.value/max)*100)}%`}}/><span className="absolute -bottom-4 left-1/2 -translate-x-1/2 font-mono text-[7px] text-frost2">{obs.value}</span></div>})}</div><div className="mt-4 flex items-center justify-between gap-2 text-[8px] text-frost2"><span>Oldest → newest</span><span>{observations.length} official observations</span></div></>:<p className="mt-3 text-xs text-frost2">Loading official game history…</p>}</div><div className="mt-3 rounded-2xl border border-edge/20 bg-edge/[0.035] p-4"><p className="text-[9px] font-black uppercase text-edge">AI takeaway</p><p className="mt-2 text-xs leading-relaxed text-frost">{leg.justification||research?.reason||'Request a current evidence-backed rationale from the analyst.'}</p><button onClick={()=>onSend(`Give me a full evidence-backed breakdown of ${leg.selection??name}, including recent games, injuries, market price, model probability, and risk.`)} className="mt-3 rounded-lg bg-edge px-3 py-2 text-[9px] font-black text-ink">Refresh evidence</button></div></Shell>;
}
...
function VoiceView({ onSend }: { onSend:(text:string)=>void }) {
  ...
  const speechWindow=window as typeof window & { SpeechRecognition?: new()=>any; webkitSpeechRecognition?: new()=>any }; const Recognition=speechWindow.SpeechRecognition??speechWindow.webkitSpeechRecognition;
...
function PregameView({ health, healthInfo, analyst, onSend }: { health:HealthState; healthInfo:HealthInfo|null; analyst?:AnalystState; onSend:(text:string)=>void }) {
  ...
  return <Shell><Brand/><div className="mt-5"><LiveBadges health={health} healthInfo={healthInfo}/></div><div className="mt-4 rounded-2xl border border-line bg-panel/70 p-5"><p className="text-[9px] font-black uppercase text-edge">Pregame Intelligence Brief</p><h2 className="mt-2 text-2xl font-black text-head">Research one matchup deeply.</h2><p className="mt-2 text-xs leading-relaxed text-frost2">Uses the existing schedule, injury, lineup, odds, weather and stats tools. Missing data must be called out instead of fabricated.</p><div className="mt-5 flex gap-2"><input value={matchup} onChange={e=>setMatchup(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')launch()}} placeholder="e.g. Celtics vs Nuggets tonight" className="min-w-0 flex-1 rounded-xl border border-line bg-ink/70 px-3 py-3 text-sm text-head outline-none focus:border-edge/50"/><button onClick={launch} disabled={!matchup.trim()} className="rounded-xl bg-edge px-4 py-3 text-[10px] font-black uppercase text-ink disabled:opacity-40">Research</button></div></div><div className="mt-3 grid grid-cols-2 gap-2">{[['Injuries','Current status'],['Probable lineup','Event context'],['Line movement','Market context'],['Matchup stats','Official feeds'],['Sportsbooks','Available prices'],['What matters most','Grounded conclusion']].map(([a,b])=><div key={a} className="rounded-xl border border-line bg-panel/55 p-3"><p className="text-[9px] font-black uppercase text-head">{a}</p><p className="mt-1 text-[9px] text-frost2">{b}</p></div>)}</div>{analyst?.prompt?<div className="mt-3 rounded-2xl border border-edge/20 bg-panel/75 p-4"><div className="flex items-center justify-between gap-3"><p className="text-[9px] font-black uppercase text-edge">Live analyst brief</p><span className="text-[8px] text-frost2">{analyst.streaming?'Working…':analyst.status||'Complete'}</span></div><p className="mt-2 whitespace-pre-wrap text-xs leading-relaxed text-frost">{analyst.answer||(analyst.streaming?'Gathering and validating source data…':'No narrative answer was returned.')}</p></div>:null}</Shell>;
}
...
export default function ExperienceHub(props: Props) {
  const [active,setActive]=useState<ExperienceId>('parlay'); const primary=props.legs[0];
  const view=()=>{switch(active){
    ...
    case'pregame':return <PregameView health={props.health} healthInfo={props.healthInfo} analyst={props.analyst} onSend={props.onSend}/>;
