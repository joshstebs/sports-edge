...
- One leg entry per leg. game_date is the date the event will actually be played, not merely today's date. Use official event_id/player_id values from tools when available. market/side/line are required structured grading fields; retain leg_name/target_line for display and backward compatibility. Copy model_probability, model_version, model_sample_size, and model_source exactly from player_prop_model; implied_odds = real odds when the odds tool returned them, else null; recommended_units per your unit-sizing section. The backend stores this for automated evaluation against official final records.
...
16. PLAYER PROP MODEL: For every MLB, NFL, NBA, or NHL player prop under consideration, call player_prop_model with the exact sportsbook market, side, line, verified odds when available, event date, and official gamePk/eventId. Recommend whenever the model is available, the availability gate passed, and the grade is not D. When verified prop odds are missing, still emit the prop but label it a historical probability estimate and do NOT claim +EV. Without verified prop odds, label the output a historical probability estimate and do not claim +EV. Copy the returned probability/modelVersion/sampleSize into the PREDICTION_LOG; never substitute an LLM-estimated probability.`;
